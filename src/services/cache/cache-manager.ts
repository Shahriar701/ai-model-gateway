...
export class CacheManager {
  ...
  async cacheLLMResponse(requestHash: string, response: any, ttlSeconds?: number): Promise<void> {
    ...
    try {
      ...
      await this.cacheService.set(key, value, ttlSeconds || this.config.defaultTtlSeconds);
   ...
   * Cache product data
   */
  async cacheProductData(productId: string, productData: any, ttlSeconds?: number): Promise<void> {
    if (!this.cacheService) return;
    ...
    try {
      ...
      logger.debug('Cached product data', {
        productId,
        ttl: ttlSeconds || this.config.defaultTtlSeconds,
      });
    } catch (error) {
      logger.error(
        `Failed to cache product data for ${productId}`,
        error instanceof Error ? error : new Error(String(error))
      );
    }
  }
  ...
  async getCachedProductData(productId: string): Promise<any | null> {
    ...
    try {
      ...
      logger.error(
        `Failed to get cached product data for ${productId}`,
        error instanceof Error ? error : new Error(String(error))
      );
      return null;
    }
  ...
  async cacheProviderHealth(
    ...
    try {
      ...
      logger.error(
        `Failed to cache provider health for ${provider}`,
        error instanceof Error ? error : new Error(String(error))
      );
    }
  }
   ...
   */
  async getCachedProviderHealth(
    provider: string
  ): Promise<{ healthy: boolean; lastCheck: number } | null> {
    if (!this.cacheService) return null;
    ...
    try {
      ...
      logger.error(
        `Failed to get cached provider health for ${provider}`,
        error instanceof Error ? error : new Error(String(error))
      );
      return null;
    }
  ...
  async invalidateCache(namespace: string, pattern?: string): Promise<void> {
    ...
    try {
      ...
      logger.error(
        `Failed to invalidate cache for namespace ${namespace}`,
        error instanceof Error ? error : new Error(String(error))
      );
    }
  }
